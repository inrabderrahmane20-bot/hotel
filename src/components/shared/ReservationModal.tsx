...
export function ReservationModal({ open, onClose }: Props) {
  const [form, setForm] = useState<FormState>({ name: '', email: '', phone: '', date: '', time: '', guests: '2 Guests', notes: '' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    onClose();
    toast.success('Table reserved!', { description: `Confirmed for ${form.date} at ${form.time}. See you soon, ${form.name.split(' ')[0]}!` });
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg w-full max-h-[92vh] overflow-y-auto mx-2 sm:mx-auto">
        <DialogHeader>
          <DialogTitle className="text-[#7C8A5D] text-xl">Reserve a Table</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground/60 flex items-center gap-1.5"><User size={12} /> Full Name</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-medium text-foreground/60 flex items-center gap-1.5"><Calendar size={12} /> Date</label>
              <input type="date" value={form.date} onChange={set('date')} min={today} className={inputCls('date')} />
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-foreground/60 flex items-center gap-1.5"><Clock size={12} /> Preferred Time</label>
            {TIME_SLOTS.map((group) => (
              <div key={group.label}>
                <p className="text-xs text-foreground/40 mb-1.5">{group.label}</p>
                <div className="flex flex-wrap gap-2">
                  {group.slots.map((slot) => (
                    <button key={slot} type="button" onClick={() => setForm((f) => ({ ...f, time: slot }))}
                      className={`px-3 py-1.5 rounded-lg text-sm border transition-colors min-w-[64px] ${form.time === slot ? 'bg-[#7C8A5D] text-white border-[#7C8A5D]' : 'border-border hover:border-[#7C8A5D] hover:text-[#7C8A5D]'}`}>
                      {slot}
                    </button>
                  ))}
                </div>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full bg-[#7C8A5D] text-white py-3 rounded-lg hover:bg-[#5A6842] transition-colors shadow-md font-medium disabled:opacity-60 text-sm">
